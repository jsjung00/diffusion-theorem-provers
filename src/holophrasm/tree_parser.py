...
class file_contents:
    def __init__(self, initial_file_name='../data/set.mm'):
...
class proof_step:
    def __init__(self, tree, context, prop, prior_statements):
        ...
        self.pos_in_context = None
        self.pos_in_proof_list = None
        ...
        self.descendants = 1 + sum([p.descendants for p in prior_entails])  # the number of descendants in the proof tree, including this node
...
class proposition:
    ...
    def __init__(self,a_or_p,label,block,statement, proof, number=None, tree=None):
        ...
        self.entails_proof_steps_pos = None
    ...
    def unify(self,trees,context):
        ...
        for v1,v2 in self.d:
            if v1 not in var_sets or v2 not in var_sets:
                #print (self.label, v1, v2, 'missing variables')
                continue # one of these is an optional variable, used only in the proof
            for x1 in var_sets[v1]:
                for x2 in var_sets[v2]:
                    assert (x1,x2) in context.d or (x2,x1) in context.d
                    added_d.append(x1, x2)
        ...
        return self.tree.copy().replace(replacement_dict), added_d
